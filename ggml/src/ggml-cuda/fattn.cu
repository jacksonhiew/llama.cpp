...
static void ggml_cuda_flash_attn_ext_mma_f16_switch_ncols1(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    // sm_75: head_dim > 128 spills more with a 64-token Q tile, so keep tile32
    // through 8K KV. Above that, fewer KV-loop iterations outweigh the spill cost.
    // head_dim <= 128 has no measured spill cliff and can use tile64 once the small-Q
    // dispatches above no longer apply. See ggml-org/llama.cpp#28761.
    const bool is_turing = GGML_CUDA_CC_IS_NVIDIA(cc) &&
        ggml_cuda_highest_compiled_arch(cc) == GGML_CUDA_CC_TURING;
    const bool turing_prefers_tile32 = is_turing && DKQ > 128 && K->ne[1] <= 8192;

    if (Q->ne[1] <= 32/ncols2 || turing_prefers_tile32 ||
        ...
        if (can_use_vector_kernel) {
            if (!ggml_is_quantized(K->type) && !ggml_is_quantized(V->type)) {
                ...
                if (cc >= GGML_CUDA_CC_ADA_LOVELACE && Q->ne[1] == 1 && Q->ne[3] == 1 &&
                        !(gqa_ratio > 4 && K->ne[1] >= 8192) && !sparse_decode) {
