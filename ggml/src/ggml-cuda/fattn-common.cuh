...
#pragma unroll
        for (int k_KQ_1 = 0; k_KQ_1 < cpy_ne; ++k_KQ_1) {
            ...
            // the same qs byte and the same signs byte.
...
// elements share one qs byte and one signs byte, loaded once.
template <typename T, int ne>
...
void launch_fattn(
    ...
    if (use_sparse) {
        ...
        ggml_cuda_flash_attn_ext_compact_mask(
            mask, KV_max.ptr, KV_max.ptr + size_t(n_kv_max)*n_lists,
            Q->ne[1], ncols1, n_kv_max, main_stream);
    }
