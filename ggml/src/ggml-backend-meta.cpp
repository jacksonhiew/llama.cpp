    ...
    if (needs_rebuild) {
...

        {
            ...
            if (i_start < cgraph->n_nodes) {
                ggml_tensor * node = cgraph->nodes[cgraph->n_nodes - 1];
                GGML_ASSERT(node->view_src != nullptr && node->view_src->op == GGML_OP_NONE && ggml_backend_buffer_is_host(node->view_src->buffer));
