...
| GGML_CUDA_FA_QUANTS           | `all` or `type_K-type_V` list | standard and TurboQuant KV pairs | Select which K/V type combinations to compile the FlashAttention CUDA kernels for. `all` compiles every base-type pair and the supported TurboQuant KV pairs, but compilation takes much longer. Otherwise a `;`-separated list of `type_K-type_V` pairs; f16-f16 is always compiled. Combinations that were not compiled fall back to f16-f16 kernel with a warning. Legal types: f16, bf16, q4_0, q4_1, q5_0, q5_1, q8_0, turbo2_0, turbo3_0, turbo4_0. |
