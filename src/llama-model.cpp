        ...
        case LLM_ARCH_QWEN35:
            {
                ...
                // The MTP block runs with no-cache attention at decode time, so it does
                // not need its own KV slot (matches GLM4 pattern).
                ml.get_key(LLM_KV_NEXTN_PREDICT_LAYERS, hparams.nextn_predict_layers, false);
                GGML_ASSERT(hparams.nextn_predict_layers < hparams.n_layer && "nextn_predict_layers must be < n_layer");
                hparams.n_layer_kv_from_start = hparams.n_layer - hparams.nextn_predict_layers;
        ...
        case LLM_ARCH_QWEN35MOE:
            {
                ...
                // NextN/MTP (Qwen3.5/3.6 MoE): extra decoder block appended beyond the main stack.
                // The MTP block is dense attention-only at decode time and does not own a KV slot.
                ml.get_key(LLM_KV_NEXTN_PREDICT_LAYERS, hparams.nextn_predict_layers, false);
                GGML_ASSERT(hparams.nextn_predict_layers < hparams.n_layer && "nextn_predict_layers must be < n_layer");
                hparams.n_layer_kv_from_start = hparams.n_layer - hparams.nextn_predict_layers;
